...
/**
 * Created by Salim on 25/04/2015.
 */
public class Fragment_this_week_homework extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState){
        ...
        if(savedInstanceState == null) {
            if(getArguments() != null)
                weekOfYear = getArguments().getInt(Fragment_home.DATE);
        ...
        while(homework.moveToNext()){
            ...
            if(hw_week_of_year == weekOfYear) {
                if (calendar.before(dateValidation.formatDateANDTimeInPm(storedDate + " " + storedTimeLimit))) {
                    ...
                    events.add(new EventData(storedTitle, storedDescription, storedDate, storedTimeLimit, remainingTime, 'H', storedCourse, storedColor));
                }
            }
    ...
    public void addEvents() {
        for (int i = 0; i < events.size(); i++) {
            if(events.get(i).type == 'H'){
                ...
                arguments.putBoolean(THIS_WEEK, true);
